...
<div class="min-h-screen max-w-md mx-auto flex flex-col">
        ...
        <header class="mb-8 text-center">
            <h1 class="text-2xl font-semibold text-brand-900 tracking-tight">{{ Auth::user()->first_name }} {{ Auth::user()->last_name }}</h1>
            <span class="text-brand-400 text-base">-- {{ '@'.Auth::user()->username }} --</span>
        </header>
        @if(session('success'))
            <div class="mb-4 p-3 bg-brand-100 text-brand-900 text-center rounded-xl border border-brand-200 animate-fade-in">{{ session('success') }}</div>
        @endif
            ...
            <form id="sortForm" method="POST" action="{{ route('mylinks.reorder') }}">
                ...
                <ul id="sortableLinks" class="flex flex-col gap-6">
                    @forelse($links as $link)
                        <li class="group flex items-center gap-3 bg-white border border-brand-100 shadow-sm rounded-xl p-2 transition-all duration-200 hover:border-brand-300 hover:shadow-md cursor-pointer" data-id="{{ $link->id }}">
                            ...
                            <button type="button" class="drag-handle cursor-grab bg-brand-50 hover:bg-brand-100 text-brand-400 px-2 py-2 rounded-lg focus:outline-none shadow-sm" aria-label="Sortieren" tabindex="0">
                            ...
                            <div class="flex-1 flex flex-col items-center w-full">
                                <a href="{{ $link->url }}" target="_blank" rel="noopener" class="block w-full text-lg font-semibold text-brand-900 bg-white hover:bg-brand-50 px-0 py-2 text-center rounded-lg transition-all duration-150 mb-1 tracking-tight">{{ $link->title }}</a>
                                <span class="block w-full text-xs text-brand-300 text-center truncate">{{ $link->url }}</span>
                            </div>
                            <div class="relative flex-shrink-0 ml-2">
                                <button type="button" class="dropdown-toggle bg-brand-50 hover:bg-brand-100 text-brand-400 px-2 py-2 rounded-lg focus:outline-none shadow-sm" aria-label="Aktionen" tabindex="0">
                                ...
                                <div class="dropdown-menu absolute right-0 mt-2 w-32 bg-white border border-brand-100 rounded-xl shadow z-50 hidden">
                                    <button type="button" class="edit-link-btn w-full text-left px-4 py-2 text-brand-900 hover:bg-brand-50" data-link-id="{{ $link->id }}">Bearbeiten</button>
                                    <form method="POST" action="{{ route('mylinks.destroy', $link) }}" onsubmit="return confirm('Diesen Link wirklich löschen?');" class="w-full">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="w-full text-left px-4 py-2 text-red-400 hover:bg-brand-50">Löschen</button>
                                    </form>
                                </div>
                            </div>
                        </li>
                        <li id="edit-form-{{ $link->id }}" class="hidden w-full">
                            <form method="POST" action="{{ route('mylinks.update', $link) }}" class="flex flex-col gap-2 bg-white border border-brand-100 rounded-xl p-4 shadow mt-2">
                                @csrf
                                <input type="hidden" name="_method" value="PUT">
                                <input type="text" name="title" value="{{ $link->title }}" required class="border border-brand-100 px-2 py-1 bg-white text-brand-900 w-full text-sm rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-200 placeholder-gray-400 transition-all duration-150" aria-label="Titel bearbeiten">
                                <input type="url" name="url" value="{{ $link->url }}" required class="border border-brand-100 px-2 py-1 bg-white text-brand-900 w-full text-sm rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-200 placeholder-gray-400 transition-all duration-150" aria-label="URL bearbeiten">
                                <div class="flex gap-2 justify-end">
                                    <button type="submit" class="bg-brand-500 hover:bg-brand-600 text-white px-3 py-1 font-medium text-xs rounded-lg shadow">Ändern</button>
                                    <button type="button" class="close-edit-form bg-brand-50 hover:bg-brand-100 text-brand-900 px-3 py-1 font-medium text-xs rounded-lg shadow" data-link-id="{{ $link->id }}">Abbrechen</button>
                                </div>
                            </form>
                        </li>
                        </li>
                    @empty
                        <li class="text-center text-brand-400">Du hast noch keine Links hinzugefügt.</li>
                    @endforelse
                </ul>
                <button type="button" id="saveOrderBtn" class="mt-6 w-full bg-brand-600 hover:bg-brand-700 text-white px-6 py-2 font-semibold rounded-lg shadow">Sortierung speichern</button>
                <div id="orderStatus" class="mt-2 text-center text-sm"></div>
            </form>
        </section>
</div>
<footer class="fixed bottom-0 left-0 w-full bg-white/80 border-t border-brand-200 rounded-t-3xl shadow-2xl flex flex-col items-center py-4 z-40 backdrop-blur-md">
    <button id="openNewLinkPanel" class="bg-brand-600 hover:bg-brand-700 text-white px-6 py-2 rounded-xl font-semibold text-base shadow transition-all duration-150 mb-2">Neuer Link</button>
</footer>

<div id="newLinkMask" class="fixed inset-0 z-50 hidden backdrop-blur-md bg-black/40 transition-opacity duration-300"></div>
<form id="newLinkFooterForm" method="POST" action="{{ route('mylinks.store') }}" class="flex flex-col gap-4 w-full p-6 max-w-md mx-auto new-link-animate bg-white/90 border border-brand-200 rounded-2xl shadow-2xl backdrop-blur-md" style="display:none;">
    @csrf
    <input type="text" name="title" placeholder="Titel" required class="border border-brand-300 px-4 py-2 bg-white text-brand-900 w-full rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-400 placeholder-gray-400 transition-all duration-150" aria-label="Titel">
    <input type="url" name="url" placeholder="https://dein-link.de" required class="border border-brand-300 px-4 py-2 bg-white text-brand-900 w-full rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-400 placeholder-gray-400 transition-all duration-150" aria-label="URL">
    <div class="flex gap-2 justify-center">
        <button type="submit" class="bg-brand-600 hover:bg-brand-700 text-white px-6 py-2 rounded-lg font-semibold shadow">Speichern</button>
        <button type="button" id="closeNewLinkFooterForm" class="bg-brand-100 hover:bg-brand-200 text-brand-900 px-6 py-2 rounded-lg font-semibold shadow">Abbrechen</button>
    </div>
</form>
<style>
